...
class SimplifiedWTFD1D(nn.Module):
    """
    简化的1D小波变换高低频特征分解模块
    去除冗余的高频分量处理，保留核心功能
    """
    def __init__(self, in_ch, out_ch):
        super(SimplifiedWTFD1D, self).__init__()
    ...
    def dwt_1d(self, x):
        ...
        high_freq = x_padded[:, :, 1:] - x_padded[:, :, :-1]
        high_freq = F.avg_pool1d(high_freq, kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        # 简化处理
        low_out = self.low_freq_conv(low_freq)
...
class LightweightEncoder(nn.Module):
    """
    轻量级编码器 - 简化小波处理，专注于有效特征提取
    """
    def __init__(self, configs):
        super(LightweightEncoder, self).__init__()
        
        # 单层WTFD - 避免过度分解
        self.wtfd = SimplifiedWTFD1D(configs.d_model, configs.d_model)
        ...
        # 简化的融合权重
        self.fusion_weight = nn.Parameter(torch.tensor(0.3))  # 小波权重
    ...
    def forward(self, x, attn_mask=None):
        ...
        low_out, high_out = self.wtfd(x_conv)
        
        # 特征融合
        if low_out.size(-1) != high_out.size(-1):
            target_size = min(low_out.size(-1), high_out.size(-1))
            low_out = F.adaptive_avg_pool1d(low_out, target_size)
            high_out = F.adaptive_avg_pool1d(high_out, target_size)
        
        combined = torch.cat([low_out, high_out], dim=1)  # (B, 2*C, L')
        wavelet_features = self.feature_fusion(combined)  # (B, C, L')
        
        # 恢复到原始长度
        if wavelet_features.size(-1) != L:
            wavelet_features = F.interpolate(wavelet_features, size=L, mode='linear', align_corners=True)
        
        wavelet_features = wavelet_features.permute(0, 2, 1)  # (B, L, C)
        ...
        output = weight * wavelet_features + (1 - weight) * transformer_out
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # 轻量级编码器 (优化版本)
        self.encoder = LightweightEncoder(configs)
