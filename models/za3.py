...
class TimeSeriesAttention(nn.Module):
    """
    专为时序预测设计的注意力机制
    基于3.py中的Attention类，针对时序数据进行优化
    """
    def __init__(self, d_model, num_heads, head_dim=None, dropout=0.1, 
                 temporal_bias=True, causal_mask=False):
        super().__init__()
        ...
        # Q, K, V投影层
        self.to_q = nn.Linear(d_model, self.dim_inner, bias=False)
        self.to_k = nn.Linear(d_model, self.dim_inner, bias=False)
        self.to_v = nn.Linear(d_model, self.dim_inner, bias=False)
        self.to_out = nn.Linear(self.dim_inner, d_model, bias=True)
        
        # 时序位置偏置
        if self.temporal_bias:
            self.temporal_bias_layer = nn.Parameter(torch.randn(1, num_heads, 1, 1))
    ...
    def forward(self, x, encoder_hidden_states=None, attn_mask=None):
        """
        Args:
            x: [B, L, D] 输入时序数据
            encoder_hidden_states: 可选的编码器隐状态
            attn_mask: 注意力掩码
        """
        ...
        # 添加时序偏置
        if self.temporal_bias:
            attn_scores = attn_scores + self.temporal_bias_layer
        ...
        # 重塑回原始格式
        out = out.transpose(1, 2).contiguous().view(batch_size, seq_len, self.dim_inner)
...
class EnhancedTimeSeriesAttentionLayer(nn.Module):
    """增强的时序注意力层，包含残差连接和层归一化"""
    def __init__(self, d_model, num_heads, head_dim=None, dropout=0.1, 
                 temporal_bias=True, causal_mask=False):
        super().__init__()
        self.attention = TimeSeriesAttention(
            d_model, num_heads, head_dim, dropout, temporal_bias, causal_mask
        )
    ...
    def forward(self, x, encoder_hidden_states=None, attn_mask=None):
        # 注意力子层
        attn_out = self.attention(x, encoder_hidden_states, attn_mask)
...
class Model(nn.Module):
    """
    Za3: AdaWaveNet with Enhanced Time Series Attention
    融合了3.py中的注意力机制和AdaWaveNet的小波变换
    """
    ...
    def __init__(self, configs):
        ...
        # 增强的时序注意力层
        self.enhanced_attention_layers = nn.ModuleList([
            EnhancedTimeSeriesAttentionLayer(
                d_model=configs.d_model,
                num_heads=configs.n_heads,
                dropout=configs.dropout,
                temporal_bias=True,
                causal_mask=False  # 对于预测任务，通常不需要因果掩码
            ) for _ in range(2)  # 添加2层增强注意力
        ])
    ...
    def apply_enhanced_attention(self, x):
        """应用增强的时序注意力"""
        for attention_layer in self.enhanced_attention_layers:
            x = attention_layer(x)
        return x
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, clusters):
        ...
        # 应用增强的时序注意力
        enc_out = self.apply_enhanced_attention(enc_out)
