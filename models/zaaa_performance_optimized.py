...
class ImprovedDCTAttention(nn.Module):
    """
    基于ZAAA_Final的DCT注意力，保持高性能的同时轻微简化
    """
    def __init__(self, d_model, num_heads=8, dropout=0.1):
        super(ImprovedDCTAttention, self).__init__()
        self.d_model = d_model
        self.num_heads = num_heads
        self.head_dim = d_model // num_heads
        
        # 双路径频域处理 - 保持ZAAA_Final的核心设计
        self.freq_conv_3x3 = nn.Sequential(
            nn.Conv1d(d_model, d_model, kernel_size=3, padding=1, groups=d_model),
            nn.BatchNorm1d(d_model),
            nn.GELU(),
        )
        
        self.freq_conv_5x5 = nn.Sequential(
    ...
    def forward(self, x):
        ...
        # 频域处理
        x_freq = x.permute(0, 2, 1)  # (B, C, L)
        freq_3x3 = self.freq_conv_3x3(x_freq)
        freq_5x5 = self.freq_conv_5x5(x_freq)
        
        # 融合频域特征
        freq_combined = torch.cat([freq_3x3, freq_5x5], dim=1)  # (B, 2C, L)
        freq_combined = freq_combined.permute(0, 2, 1)  # (B, L, 2C)
        freq_fused = self.fusion(freq_combined)  # (B, L, C)
        
        # 注意力机制
        attn_out, _ = self.attention(x, x, x)
        
        # 第一次残差连接
        x = self.norm1(freq_fused + attn_out + residual)
...
class OptimizedEncoder(nn.Module):
    """
    基于ZAAA_Final的优化编码器，保持高性能
    """
    def __init__(self, configs):
        super(OptimizedEncoder, self).__init__()
        
        # DCT注意力
        self.dct_attention = ImprovedDCTAttention(
            configs.d_model, 
        ...
        # 自适应权重调整
        self.adaptive_weight = nn.Parameter(torch.tensor([0.6, 0.4]))  # DCT权重更高
    ...
    def forward(self, x, attn_mask=None):
        ...
        # 门控融合
        combined = torch.cat([dct_out, transformer_out], dim=-1)
        gate = self.gate_linear(combined)
        
        # 自适应加权融合
        weights = F.softmax(self.adaptive_weight, dim=0)
        gated_dct = gate * dct_out * weights[0]
        gated_transformer = (1 - gate) * transformer_out * weights[1]
        
        output = gated_dct + gated_transformer
...
class Model(nn.Module):
    """
    ZAAA_Performance_Optimized: 基于ZAAA_Final的性能优先简化版本
    保持核心高性能组件，轻微简化以提升效率
    """
    ...
    def __init__(self, configs):
        ...
        # 性能优化编码器
        self.encoder = OptimizedEncoder(configs)
