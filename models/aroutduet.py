...
class TrendExpert(nn.Module):
    """
    Single trend expert used by TrendMoEExtractor.
    """
...
class TrendMoEExtractor(nn.Module):
    """
    Prototype soft-router MoE extractor for the trend component, followed by an MLP.
    Input:  (B, L, C)
    Output: (B, pred_len, C)
    """
    def __init__(self, seq_len, pred_len, enc_in, d_model=512, dropout=0.1,
                 num_experts=4, expert_expansion=2, mlp_expansion=2, router_temperature=1.0):
        super().__init__()
        self.seq_len = seq_len
        self.pred_len = pred_len
        self.enc_in = enc_in
        ...
        self.experts = nn.ModuleList([
            TrendExpert(d_model=d_model, dropout=dropout, expansion=expert_expansion)
        ...
        self.expert_prototypes = nn.Parameter(torch.empty(num_experts, d_model))
        self.router_temperature = nn.Parameter(torch.tensor(float(router_temperature)))
        self.register_buffer('expert_gate_sum', torch.zeros(num_experts))
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.trend_extractor = TrendMoEExtractor(
            ...
            num_experts=getattr(configs, 'trend_num_experts', 4),
