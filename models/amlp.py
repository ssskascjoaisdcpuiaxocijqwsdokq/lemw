...
class Adaptive_ECA_MRFP(nn.Module):
    """
    自适应门控版 ECA+MRFP：内部用原 ECABlock1D_MRFP，输出与输入做门控残差融合。
    gate 初值为 0，初始等价恒等映射，训练后自动放大/缩小增强块作用。
    """
    def __init__(self, channel=512, kernel_size=3, dropout=0.1, mrfp_ratio=2.0, gate_init=0.0):
        super().__init__()
        self.inner = ECABlock1D_MRFP(channel=channel, kernel_size=kernel_size, dropout=dropout, mrfp_ratio=mrfp_ratio)
        self.gate = nn.Parameter(torch.tensor(gate_init))

    def forward(self, x):
        feat = self.inner(x)
        return x + self.gate * (feat - x)


class Weak_ECA_MRFP(nn.Module):
    """
    弱化版 ECA + MRFP：以线性为主，局部卷积微扰，阻尼注意力，初始接近恒等映射。
    """
    def __init__(self, c_in, d_model, scale_init=1e-3):
        super().__init__()
        # 1) 线性/弱卷积分支
        self.conv_1x1 = nn.Conv1d(c_in, c_in, kernel_size=1)
        self.conv_3x3 = nn.Sequential(
            nn.Conv1d(c_in, c_in, kernel_size=3, padding=1, groups=c_in),
        )
        self.fusion = nn.Conv1d(c_in * 2, c_in, kernel_size=1)

        # 2) 阻尼版 ECA
        gamma = 2
        b = 1
        t = int(abs((math.log(c_in, 2) + b) / gamma))
        k_eca = t if t % 2 else t + 1
        self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.eca_conv = nn.Conv1d(1, 1, kernel_size=k_eca, padding=(k_eca - 1) // 2, bias=False)

        # 3) 可学习残差缩放，初始极小，保持近似恒等
        self.alpha = nn.Parameter(torch.tensor(scale_init))

    def forward(self, x):
        # x: (B, C, L)
        feat_linear = self.conv_1x1(x)
        feat_local = self.conv_3x3(x)
        x_fused = self.fusion(torch.cat([feat_linear, feat_local], dim=1))

        y = self.avg_pool(x_fused)          # (B, C, 1)
        y = y.transpose(-1, -2)             # (B, 1, C)
        y = self.eca_conv(y)                # (B, 1, C)
        scale = torch.sigmoid(y).transpose(-1, -2)  # (B, C, 1)
...
class LinearTrendExpert(nn.Module):
    """
    Linear expert for simple trend mapping.
    """
...
class MLPTrendExpert(nn.Module):
    """
    Nonlinear MLP expert for complex trend patterns.
    """
    def __init__(self, d_model, dropout=0.1, expansion=2):
        super().__init__()
...
class ResidualCorrectionTrendExpert(nn.Module):
    """
    Residual correction expert that learns a small nonlinear delta over the input.
    """
    def __init__(self, d_model, dropout=0.1, expansion=2, scale_init=0.1):
        super().__init__()
        hidden_dim = int(d_model * expansion)
    ...
    def forward(self, x):
        return x + self.scale * self.correction(x)


class TrendMoEExtractor(nn.Module):
    """
    Prototype soft-router MoE extractor with linear, MLP, and residual correction experts.
    Input:  (B, L, C)
    Output: (B, pred_len, C)
    """
    def __init__(self, seq_len, pred_len, enc_in, d_model=512, dropout=0.1,
                 num_experts=3, expert_expansion=2, mlp_expansion=2, router_temperature=1.0):
        super().__init__()
        self.seq_len = seq_len
        self.pred_len = pred_len
        self.enc_in = enc_in
        ...
        self.experts = nn.ModuleList([
            LinearTrendExpert(d_model=d_model, dropout=dropout),
            MLPTrendExpert(d_model=d_model, dropout=dropout, expansion=expert_expansion),
            ResidualCorrectionTrendExpert(d_model=d_model, dropout=dropout, expansion=expert_expansion)
        ])
        ...
        self.expert_prototypes = nn.Parameter(torch.empty(self.num_experts, d_model))
        self.router_temperature = nn.Parameter(torch.tensor(float(router_temperature)))
        self.register_buffer('expert_gate_sum', torch.zeros(self.num_experts))
        self.register_buffer('expert_gate_count', torch.zeros(1))

        mlp_hidden = int(d_model * mlp_expansion)
    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                ...
                if m.bias is not None:
                    init.constant_(m.bias, 0)
        init.xavier_uniform_(self.expert_prototypes)

    def route(self, x_reshaped):
        route_stats = torch.cat([
            x_reshaped.mean(dim=-1, keepdim=True),
            x_reshaped.std(dim=-1, keepdim=True, unbiased=False),
            x_reshaped.max(dim=-1, keepdim=True).values,
            x_reshaped.min(dim=-1, keepdim=True).values,
            (x_reshaped[:, -1:] - x_reshaped[:, :1])
        ], dim=-1)

        route_features = self.route_proj(route_stats)
        distances = torch.cdist(route_features, self.expert_prototypes)
        temperature = torch.clamp(self.router_temperature.abs(), min=1e-3)
    ...
    def forward(self, x):
        ...
        x_norm = self.revin(x, 'norm')
        x_reshaped = rearrange(x_norm, 'b l c -> (b c) l')
        temporal_features = self.temporal_projection(x_reshaped)

        gate_weights = self.route(x_reshaped)
        if not self.training:
            self.expert_gate_sum += gate_weights.detach().sum(dim=0)
            self.expert_gate_count += gate_weights.shape[0]

        expert_outputs = torch.stack(
            [expert(temporal_features) for expert in self.experts],
            dim=1
        )
        moe_features = torch.sum(expert_outputs * gate_weights.unsqueeze(-1), dim=1)

        mlp_features = self.post_mlp(moe_features)
        predictions = self.prediction_head(mlp_features)
        predictions = rearrange(predictions, '(b c) p -> b p c', b=B, c=C)
        predictions = self.revin(predictions, 'denorm')
    ...
    def reset_expert_usage(self):
        self.expert_gate_sum.zero_()
        self.expert_gate_count.zero_()

    def get_expert_usage(self):
        if self.expert_gate_count.item() == 0:
            return None
        return self.expert_gate_sum / self.expert_gate_count
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # MoE trend extractor followed by an MLP.
        self.trend_extractor = TrendMoEExtractor(
            ...
            num_experts=3,
            expert_expansion=getattr(configs, 'trend_expert_expansion', 2),
            mlp_expansion=getattr(configs, 'trend_mlp_expansion', 2),
            router_temperature=getattr(configs, 'router_temperature', 1.0)
        )
        
        # 自适应门控版 ECA+MRFP（初始 gate=0，等价恒等映射）
