...
def normalization(channels: int):
    return nn.LayerNorm(channels)  # 使用LayerNorm替代InstanceNorm1d，更稳定

class OptimizedSEAttention1D(nn.Module):
    """
    优化的SE注意力机制 - 高效的1D时序数据处理
    改进：
    1. 使用更高效的全局池化
    2. 添加残差连接和层归一化
    3. 优化权重初始化
    4. 支持动态reduction ratio
    """
    def __init__(self, channel=512, reduction=16, num_inputs=3):
        ...
        # 更高效的全局池化：同时使用平均池化和最大池化
        self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.max_pool = nn.AdaptiveMaxPool1d(1)
        
        # 优化的SE网络：添加BatchNorm和更好的激活函数
        self.fc = nn.Sequential(
            nn.Linear(channel * 2, channel // self.reduction, bias=False),  # *2 因为concat了avg和max
    ...
    def forward(self, *inputs):
        ...
        # 输入融合
        x_combined = sum(inputs) / len(inputs)  # 使用平均而不是简单相加
        
        # 双重池化：平均池化和最大池化
        avg_out = self.avg_pool(x_combined).view(B, C)  # (B, C)
        max_out = self.max_pool(x_combined).view(B, C)  # (B, C)
        
        # 特征融合
        y = torch.cat([avg_out, max_out], dim=1)  # (B, 2C)
        
        # SE变换
        y = self.fc(y).view(B, self.num_inputs * C, 1)  # (B, num_inputs*C, 1)
        ...
        # 加权融合
        weighted_sum = sum(inp * weight for inp, weight in zip(inputs, weights))
        
        # 残差连接和层归一化
        output = weighted_sum + self.residual_weight * x_combined
        output = output.permute(0, 2, 1)  # (B, L, C) for LayerNorm
...
class OptimizedMultiScaleConv1D(nn.Module):
    """
    优化的多尺度卷积模块
    改进：
    1. 使用深度可分离卷积减少参数
    ...
    def __init__(self, channels, dropout=0.1):
        ...
        # 使用深度可分离卷积
        self.conv1 = DepthwiseSeparableConv1D(channels, channels, 3, padding=1)  # 短期
        self.conv2 = DepthwiseSeparableConv1D(channels, channels, 5, padding=2)  # 中期
        self.conv3 = DepthwiseSeparableConv1D(channels, channels, 7, padding=3)  # 长期
        
        # 空洞卷积增加感受野
        self.dilated_conv = nn.Conv1d(channels, channels, 3, padding=2, dilation=2)
        
        # 优化的SE注意力机制
        self.se_attention = OptimizedSEAttention1D(
            channel=channels, 
    ...
    def forward(self, x):
        ...
        """
        # 多尺度特征提取
        f1 = F.silu(self.conv1(x))  # 短期模式
        f2 = F.silu(self.conv2(x))  # 中期模式
        f3 = F.silu(self.conv3(x))  # 长期模式
        f4 = F.silu(self.dilated_conv(x))  # 空洞卷积
        
        # SE注意力融合
        enhanced = self.se_attention(f1, f2, f3, f4)
        
        # 特征融合
        fused = self.feature_fusion(enhanced)
        
        # 自适应残差连接
        output = self.adaptive_weight * fused + (1 - self.adaptive_weight) * x
...
class OptimizedSEEnhancedAdpWaveletBlock(nn.Module):
    """
    优化的SE增强自适应小波块
    改进：
    1. 更高效的小波变换
    ...
    def __init__(self, configs, input_size):
        super(OptimizedSEEnhancedAdpWaveletBlock, self).__init__()
        ...
        self.norm_x = nn.LayerNorm(configs.enc_in)
        self.norm_d = nn.LayerNorm(configs.enc_in)
        
        # 优化的多尺度特征提取
        self.multiscale_conv = OptimizedMultiScaleConv1D(configs.enc_in, configs.dropout)
        
        # 特征增强
        self.feature_enhance = nn.Sequential(
            nn.Conv1d(configs.enc_in, configs.enc_in, 1),
    ...
    def forward(self, x):
        ...
        # 使用LayerNorm替代InstanceNorm
        x = x.permute(0, 2, 1)  # (B, L, C)
        d = d.permute(0, 2, 1)  # (B, L, C)
        x = self.norm_x(x).permute(0, 2, 1)  # (B, C, L)
        d = self.norm_d(d).permute(0, 2, 1)  # (B, C, L)
...
class OptimizedSEEnhancedClusteredLinear(nn.Module):
    """
    优化的SE增强聚类线性层
    改进：
    1. 更高效的线性变换
    ...
    def __init__(self, n_clusters, enc_in, seq_len, pred_len):
        ...
        # 优化的SE注意力用于特征增强
        self.se_enhance = OptimizedMultiScaleConv1D(enc_in, dropout=0.1)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # 使用优化的SE增强聚类线性层
        self.trend_linear = OptimizedSEEnhancedClusteredLinear(configs.n_clusters, configs.enc_in, self.seq_len, configs.pred_len)
        ...
        for i in range(configs.lifting_levels):
            ...
            self.encoder_levels.add_module(
                ...
                OptimizedSEEnhancedAdpWaveletBlock(configs, input_size)
            )
            in_planes *= 1
            ...
            enhanced_linear = nn.Sequential(
                ...
                nn.LayerNorm(input_size * expand_ratio)
            )
