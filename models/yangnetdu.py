...
class EnhancedSeriesDecomp(nn.Module):
    """增强的序列分解 - 多尺度趋势提取"""
    def __init__(self, kernel_sizes=[12, 24, 36], imputation=False):
        super(EnhancedSeriesDecomp, self).__init__()
        self.kernel_sizes = kernel_sizes
        
        if imputation:
            self.moving_avg = EnhancedMovingAvg(kernel_sizes)
        else:
            self.moving_avg = EnhancedMovingAvg(kernel_sizes)
...
class MultiScaleFeatureEnhancement(nn.Module):
    ...
    def __init__(self, channels, dropout=0.1):
        ...
        self.attention = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(channels, channels * 3, 1),
            nn.Softmax(dim=1)
        )
...
class TemporalRefinement(nn.Module):
    ...
    def __init__(self, seq_len, pred_len, channels):
        ...
        self.refinement_net = nn.Sequential(
            nn.AdaptiveAvgPool1d(seq_len),  # 确保输入长度一致
            ...
            nn.GELU(),
            nn.Dropout(0.1),
            nn.AdaptiveAvgPool1d(pred_len)  # 输出到目标长度
        )
    ...
    def forward(self, x):
        ...
        # 残差连接（如果形状匹配）
        if refined.shape[-1] == x.shape[-1]:
            output = x + self.residual_weight * refined
        else:
            output = refined
        return output
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # 使用增强的序列分解（若缺失则回退到 Autoformer 的 series_decomp）
        try:
            self.series_decomp = EnhancedSeriesDecomp(imputation=self.task_name=='imputation')
        except NameError:
            self.series_decomp = _autoformer_series_decomp(getattr(configs, 'moving_avg', 25))
        ...
        for i in range(configs.lifting_levels):
            self.encoder_levels.add_module(
                ...
                EnhancedAdpWaveletBlock(configs, input_size)
            )
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, clusters):
        ...
        # 移除temporal_refinement调用，直接使用x_dec
        # x_dec = self.temporal_refinement(x_dec.permute(0,2,1)).permute(0,2,1)
