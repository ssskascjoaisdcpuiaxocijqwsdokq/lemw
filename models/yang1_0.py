            ...
            for _ in range(10):
                ...
                for i in range(n_clusters):
                    ...
                    if mask.sum() > 0:
                        ...
                        if i < n_samples:
                            new_centroids[i] = x[torch.randint(0, n_samples, (1,), device=device)]
                        else:
...
class Model(nn.Module):
    """YANGNet 1.0 - 使用fast_pytorch_kmeans的优化版本"""
    ...
    def __init__(self, configs):
        ...
        # ========== 优化模块 ==========
        self.multi_scale_extractor = MultiScaleFeatureExtraction(configs.enc_in, configs.dropout)
        self.efficient_attention = EfficientAttention(configs.d_model, configs.dropout)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, clusters):
        ...
        # ========== 多尺度特征提取 ==========
        x_enc_enhanced = self.multi_scale_extractor(x_enc)
        ...
        # ========== 高效注意力机制 ==========
        enc_out_enhanced = self.efficient_attention(enc_out)
