...
class TrendMultiHeadExtractor(nn.Module):
    """
    Trend extractor with a shared MLP and multiple projection heads.
    Input:  (B, L, C)
    Output: (B, pred_len, C)
    """
    def __init__(self, seq_len, pred_len, enc_in, d_model=512, dropout=0.1,
                 num_heads=4, mlp_expansion=2):
        super().__init__()
        self.seq_len = seq_len
        self.pred_len = pred_len
        self.enc_in = enc_in
        ...
        self.projection_heads = nn.ModuleList([
            nn.Linear(d_model, pred_len) for _ in range(num_heads)
        ])
        self.head_router = nn.Sequential(
            nn.Linear(d_model, max(d_model // 2, 1)),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(max(d_model // 2, 1), num_heads),
            nn.Softmax(dim=-1)
        )
        self.output_dropout = nn.Dropout(dropout)

        self.register_buffer('head_weight_sum', torch.zeros(num_heads))
        self.register_buffer('head_weight_count', torch.zeros(1))
    ...
    def forward(self, x):
        ...
        shared_features = self.shared_mlp(temporal_features)

        head_weights = self.head_router(shared_features)
        if not self.training:
            self.head_weight_sum += head_weights.detach().sum(dim=0)
            self.head_weight_count += head_weights.shape[0]

        head_outputs = torch.stack(
            [head(shared_features) for head in self.projection_heads],
            dim=1
        )
        predictions = torch.sum(head_outputs * head_weights.unsqueeze(-1), dim=1)
        predictions = self.output_dropout(predictions)
        predictions = rearrange(predictions, '(b c) p -> b p c', b=B, c=C)
        predictions = self.revin(predictions, 'denorm')

        return predictions

    def reset_head_usage(self):
        self.head_weight_sum.zero_()
        self.head_weight_count.zero_()

    def get_head_usage(self):
        if self.head_weight_count.item() == 0:
            return None
        return self.head_weight_sum / self.head_weight_count
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Shared trend MLP followed by multiple projection heads.
        self.trend_extractor = TrendMultiHeadExtractor(
            seq_len=self.seq_len,
            pred_len=configs.pred_len,
            enc_in=configs.enc_in,
            d_model=getattr(configs, 'duet_d_model', 512),
            dropout=configs.dropout,
            num_heads=getattr(configs, 'trend_num_heads', getattr(configs, 'trend_num_experts', 4)),
