...
from models.DLinear import Model as DLinearTrendModel
...
class ClusteredTrendExtractor(nn.Module):
    """
    基于 DLinear 的聚类趋势提取器
    """
    def __init__(self, configs):
        super(ClusteredTrendExtractor, self).__init__()
        self.n_clusters = getattr(configs, 'n_clusters', 1)
        ...
        self.cluster_models = nn.ModuleDict({
            str(cluster_id): DLinearTrendModel(copy.deepcopy(base_cfg))
    ...
    def forward(self, x, clusters):
        """
        x: (B, L, C)
        clusters: tensor/list with length C
        """
        B, _, C = x.shape
        outputs = []
        if torch.is_tensor(clusters):
            cluster_indices = clusters.long().cpu()
        else:
            cluster_indices = torch.tensor(clusters, dtype=torch.long)
        for channel in range(C):
            cluster_id = str(int(cluster_indices[channel].item()))
            channel_data = x[:, :, channel].unsqueeze(-1)
            pred = self.cluster_models[cluster_id].forecast(channel_data)
            outputs.append(pred)
        trend_out = torch.cat(outputs, dim=-1)
        return trend_out
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.enc_in = configs.enc_in
        self.n_clusters = getattr(configs, 'n_clusters', 1)
        self.kmeans = KMeans(n_clusters=self.n_clusters) if self.n_clusters > 1 else None
        self.clusters = None
        
        # 使用聚类后的 DLinear 趋势提取器
        self.trend_extractor = ClusteredTrendExtractor(configs)
    ...
    def _get_clusters(self, x_enc):
        if self.n_clusters <= 1 or self.kmeans is None:
            if self.clusters is None:
                self.clusters = torch.zeros(self.enc_in, dtype=torch.long, device=x_enc.device)
            return self.clusters
        x_cluster = x_enc.permute(2, 0, 1).contiguous().view(self.enc_in, -1)
        if self.training or self.clusters is None:
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, clusters):
        ...
        # 使用DUET线性提取器处理趋势分量
        moving_mean_out = self.trend_extractor(moving_mean, clusters)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        clusters = self._get_clusters(x_enc)
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
