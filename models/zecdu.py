...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # GLU 融合门控，动态调整季节与趋势分量权重
        self.fusion_gate = nn.Sequential(
            nn.Linear(configs.enc_in * 2, configs.enc_in * 2),
            nn.GLU(dim=-1)
        )
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        fusion_input = torch.cat([dec_out, moving_mean_out], dim=-1)
        dec_out = self.fusion_gate(fusion_input)
