...
# 通道-空间融合注意力机制 - 适配时序数据
class CSFBlock1D(nn.Module):
    def __init__(self, channel=512, reduction=16, kernel_size=7, seq_len=None, dropout=0.1):
        super().__init__()
        self.ChannelAttention = ChannelAttention1D(channel=channel, reduction=reduction)
        self.SpatialAttention = SpatialAttention1D(kernel_size=kernel_size)
        self.joint_channel = channel + seq_len if seq_len else channel + 96  # 默认序列长度
        mid_channels = max(1, channel // reduction)
        self.pre_norm = nn.BatchNorm1d(channel)
        self.MLP = nn.Sequential(
            nn.Conv1d(self.joint_channel, mid_channels, 1, bias=False),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Conv1d(mid_channels, self.joint_channel, 1, bias=False),
        )
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        residual = x
        x = self.pre_norm(x)
        Channel_x = self.ChannelAttention(x).reshape(B, C, 1)  # (B,C,1)
        Spatial_x = self.SpatialAttention(x).reshape(B, L, 1)  # (B,L,1)

        # 拼接,然后通过MLP建立相关性
        CS_x = torch.cat([Channel_x, Spatial_x], 1)  # (B,C+L,1)
        CS_xx = self.MLP(CS_x)  # (B,C+L,1)

        # 拆分,然后通过sigmoid得到权重表示
        Channel_x = CS_xx[:, :C, :].reshape(B, C, 1)  # (B,C,1)
        Spatial_x = CS_xx[:, C:, :].reshape(B, 1, L)  # (B,1,L)
        Channel_weight = self.sigmoid(Channel_x)
        Spatial_weight = self.sigmoid(Spatial_x)

        # 应用注意力权重
        out = x * Channel_weight * Spatial_weight  # (B,C,L)
        return residual + self.residual_scale * out  # 残差连接并带可学习缩放
...
class Model(nn.Module):
    """
    YangChanl: AdaWaveNet with Channel-Spatial Fusion Attention
    基于AdaWaveNet集成通道-空间融合注意力机制
    """
