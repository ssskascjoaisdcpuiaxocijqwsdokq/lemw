...
class SEAttention(nn.Module):
    ...
    def forward(self, x):
        # x: (B, C, T)
        B, C, T = x.size()
        # Squeeze: (B,C,T)-->avg_pool-->(B,C,1)-->view-->(B,C)
        y = self.avg_pool(x).view(B, C)
        # Excitation: (B,C)-->fc-->(B,C)-->(B, C, 1)
        y = self.fc(y).view(B, C, 1)
        # scale: (B,C,T) * (B, C, 1) == (B,C,T)
        out = x * y
        return out

class ScaledDotProductAttention(nn.Module):
    """缩放点积注意力 - 来自包/2_1_Attention.py，适配时序数据"""
    ...
    def forward(self, queries, keys, values, attention_mask=None):
        ...
        q = self.fc_q(queries).view(B, nq, self.h, self.d_k).permute(0, 2, 1, 3)  # (B,h,nq,d_k)
        k = self.fc_k(keys).view(B, nk, self.h, self.d_k).permute(0, 2, 3, 1)     # (B,h,d_k,nk)
        v = self.fc_v(values).view(B, nk, self.h, self.d_v).permute(0, 2, 1, 3)   # (B,h,nk,d_v)
...
class EnhancedAdpWaveletBlock(nn.Module):
    ...
    def __init__(self, configs, input_size):
        ...
        # 添加SE注意力机制
        self.se_attention = SEAttention(channel=configs.enc_in, reduction=8)
...
class Model(nn.Module):
    """
    YFTT: 融合AdaWaveNet、SE注意力和缩放点积注意力的时序预测模型
    """
    ...
    def __init__(self, configs):
        ...
        # 增强的Encoder，集成缩放点积注意力
        self.enhanced_attention = ScaledDotProductAttention(
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, clusters):
        ...
        # 应用增强注意力
        enc_out_enhanced = self.enhanced_attention(enc_out, enc_out, enc_out)
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        ...
        enc_out = self.enc_embedding(x_enc, None)
        enc_out_enhanced = self.enhanced_attention(enc_out, enc_out, enc_out)
        enc_out = enc_out + enc_out_enhanced
        enc_out, attns = self.encoder(enc_out, attn_mask=None)
        x_dec = self.lowrank_projection(enc_out)
