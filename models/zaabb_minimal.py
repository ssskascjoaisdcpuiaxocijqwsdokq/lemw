...
class MultiScaleDWConv1D(nn.Module):
    """
    Multi-scale Depthwise Convolution - 保留，这个模块通常有效
    """
    def __init__(self, dim, scale=(3, 5, 7)):  # 去除scale=1，减少复杂性
        super().__init__()
        self.scale = scale
        self.channels = []
...
class MinimalEncoder(nn.Module):
    ...
    def __init__(self, configs):
        super(MinimalEncoder, self).__init__()
        ...
        self.multiscale_conv = nn.Sequential(
            nn.Conv1d(configs.d_model, configs.d_model, kernel_size=3, padding=1, groups=configs.d_model),
            nn.GELU(),
    ...
    def forward(self, x, attn_mask=None):
        ...
        # 轻量级多尺度增强
        x_conv = transformer_out.permute(0, 2, 1)  # (B, C, L)
        x_conv = self.multiscale_conv(x_conv)
        x_conv = x_conv.permute(0, 2, 1)  # (B, L, C)
        
        # 简单融合
        output = self.norm(transformer_out + x_conv)
...
class Model(nn.Module):
    """
    ZAABB_Minimal: 最小化版本，去除所有可能有害的TransXNet模块
    只保留AdaWaveNet核心 + 轻量级多尺度增强
    """
    ...
    def __init__(self, configs):
        ...
        # 最小化编码器 - 去除所有复杂的TransXNet组件
        self.encoder = MinimalEncoder(configs)
