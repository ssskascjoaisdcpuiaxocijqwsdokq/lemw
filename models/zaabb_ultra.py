...
class UltraMinimalEncoder(nn.Module):
    """
    极简编码器 - 只保留最核心的Transformer + 最简单的增强
    """
    def __init__(self, configs):
        super(UltraMinimalEncoder, self).__init__()
        
        # 标准Transformer编码器（减少层数以降低复杂性）
        self.transformer_encoder = Encoder(
            [
                ...
                ) for l in range(max(2, configs.e_layers - 1))  # 减少一层
            ],
            norm_layer=torch.nn.LayerNorm(configs.d_model)
        )
        
        # 去除多尺度卷积，只保留简单的位置增强
        self.position_conv = nn.Conv1d(configs.d_model, configs.d_model, kernel_size=3, padding=1, groups=configs.d_model)
    ...
    def forward(self, x, attn_mask=None):
        ...
        # 简单的位置增强
        x_pos = transformer_out.permute(0, 2, 1)  # (B, C, L)
        x_pos = self.position_conv(x_pos)
        x_pos = x_pos.permute(0, 2, 1)  # (B, L, C)
        
        # 轻量级融合
        output = self.norm(transformer_out + self.dropout(x_pos))
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # 优化小波层级 - 减少到3层以降低复杂性
        self.lifting_levels = min(3, configs.lifting_levels)
        
        # Embedding
        self.enc_embedding = DataEmbedding_inverted(self.seq_len // (2 ** self.lifting_levels), configs.d_model, configs.embed, configs.freq,
                                                    configs.dropout)
        ...
        for i in range(self.lifting_levels-1, -1, -1):
        ...
        if self.task_name == "super_resolution":
            self.lowrank_projection = nn.Linear(configs.d_model, self.pred_len // (2 ** self.lifting_levels), bias=True)
        else:
            self.lowrank_projection = nn.Linear(configs.d_model, self.seq_len // (2 ** self.lifting_levels), bias=True)

        # 极简编码器
        self.encoder = UltraMinimalEncoder(configs)
