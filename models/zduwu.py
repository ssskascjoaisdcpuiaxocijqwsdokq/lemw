    ...
    def init_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv1d):
                ...
                init.constant_(m.weight, 1)
                init.constant_(m.bias, 0)
        init.constant_(self.residual_scale, 1.0)
        init.constant_(self.attention_scale, 1.0)
        init.constant_(self.mrfp_scale, 1.0)
...
class TrendLinearSimple(nn.Module):
    """去除DUET后的简单趋势线性层（按通道独立线性映射）。"""
    ...
    def forward(self, x):
        # x: (B, L, C)
        B, L, C = x.shape
        x_flat = x.permute(0, 2, 1).contiguous().view(B * C, L)
        out = self.linear(x_flat)  # (B*C, pred_len)
        out = out.view(B, C, self.pred_len).permute(0, 2, 1)
        return out
...
class moving_avg_imputation(nn.Module):
    ...
    def forward(self, x):
        ...
        window_sum = torch.nn.functional.conv1d(
            x_padded,
            weight=weight,
            stride=self.stride
        )
...
class Model(nn.Module):
    """
    ZDUWU模型: YECA + VITComer MRFP，去掉DUET线性提取器，使用简单趋势线性层。
    """
    ...
    def __init__(self, configs):
        ...
        # 简单趋势线性层（按通道独立映射）
        self.trend_extractor = TrendLinearSimple(
