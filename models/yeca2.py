...
class EnhancedTrendProcessor(nn.Module):
    ...
    def forward(self, trend_input):
        ...
        # 拼接多尺度特征
        multi_scale_concat = torch.cat(multi_scale_features, dim=1)  # (B, d_model, L)
        multi_scale_concat = multi_scale_concat.permute(0, 2, 1)  # (B, L, d_model)
        
        # 尺度融合
        fused_features = self.scale_fusion(multi_scale_concat)  # (B, L, d_model)
        
        # 残差连接
        enhanced_features = x_attended + fused_features
...
class Model(nn.Module):
    """
    YECA2: AdaWaveNet + ECA注意力 + 趋势感知注意力
    在yeca1基础上增加了专门的趋势感知注意力机制来处理趋势分量
    """
    def __init__(self, configs):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        if self.task_name == 'classification':
            raise ValueError('classification not implemented for yeca2')
        if self.task_name == 'super_resolution':
