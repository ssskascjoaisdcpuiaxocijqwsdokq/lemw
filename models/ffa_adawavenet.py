...
# ==================== FFA模块适配1D时序数据 ====================
def default_conv1d(in_channels, out_channels, kernel_size, bias=True):
    return nn.Conv1d(in_channels, out_channels, kernel_size, padding=(kernel_size // 2), bias=bias)

class PALayer1D(nn.Module):
    """像素注意力层 - 适配1D时序"""
    def __init__(self, channel):
        super().__init__()
        self.pa = nn.Sequential(
            nn.Conv1d(channel, max(channel // 8, 1), 1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv1d(max(channel // 8, 1), 1, 1, padding=0, bias=True),
            nn.Sigmoid()
        )
...
class CALayer1D(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.ca = nn.Sequential(
            nn.Conv1d(channel, max(channel // 8, 1), 1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv1d(max(channel // 8, 1), channel, 1, padding=0, bias=True),
            nn.Sigmoid()
        )
...
class Block1D(nn.Module):
    ...
    def __init__(self, conv, dim, kernel_size):
        ...
        self.calayer = CALayer1D(dim)
        self.palayer = PALayer1D(dim)
...
class Group1D(nn.Module):
    """FFA组模块 - 适配1D时序"""
    def __init__(self, conv, dim, kernel_size, blocks):
        super().__init__()
        modules = [Block1D(conv, dim, kernel_size) for _ in range(blocks)]
...
class FFA1D(nn.Module):
    """FFA特征融合注意力 - 适配1D时序数据"""
    def __init__(self, inchannel=7, blocks=1, conv=default_conv1d):
        super().__init__()
        self.gps = inchannel
        self.dim = max(inchannel, 8)  # 确保维度足够
        kernel_size = 3
        ...
        # 三个特征提取组
        self.g1 = Group1D(conv, self.dim, kernel_size, blocks=blocks)
        self.g2 = Group1D(conv, self.dim, kernel_size, blocks=blocks)
        self.g3 = Group1D(conv, self.dim, kernel_size, blocks=blocks)
        
        # 通道注意力
        self.ca = nn.Sequential(*[
            nn.AdaptiveAvgPool1d(1),
            nn.Conv1d(self.dim * 3, max(self.dim // 4, 1), 1, padding=0),
            nn.ReLU(inplace=True),
            nn.Conv1d(max(self.dim // 4, 1), self.dim * self.gps, 1, padding=0, bias=True),
            nn.Sigmoid()
        ])
        
        # 像素注意力
        self.palayer = PALayer1D(self.dim)
...
class FFAEnhancedAdpWaveletBlock(nn.Module):
    """FFA增强的自适应小波块 - 融合方案1：小波系数增强"""
    def __init__(self, configs, input_size):
        super(FFAEnhancedAdpWaveletBlock, self).__init__()
        ...
        # FFA特征融合注意力 - 用于增强小波系数
        self.ffa_coeff_enhancer = FFA1D(configs.enc_in, blocks=1)
        self.ffa_detail_enhancer = FFA1D(configs.enc_in, blocks=1)
        
        # 学习权重
        self.coeff_weight = nn.Parameter(torch.tensor(0.1))
    ...
    def forward(self, x):
        ...
        # FFA增强小波系数
        enhanced_c = self.ffa_coeff_enhancer(c)
        enhanced_d = self.ffa_detail_enhancer(d)
        
        # 自适应融合
        x = c + torch.sigmoid(self.coeff_weight) * (enhanced_c - c)
        d_enhanced = d + torch.sigmoid(self.detail_weight) * (enhanced_d - d)
        
        r = None
        if(self.regu_approx + self.regu_details != 0.0):
            if self.regu_details:
                rd = self.regu_details * d_enhanced.abs().mean()
            if self.regu_approx:
        ...
        x = self.norm_x(x)
        d_enhanced = self.norm_d(d_enhanced)
        
        return x, r, d_enhanced
...
class Model(nn.Module):
    """
    FFA-AdaWaveNet: 融合FFA特征融合注意力的AdaWaveNet模型
    融合方案1：小波系数增强
    """
    ...
    def __init__(self, configs):
        ...
        for i in range(configs.lifting_levels):
            self.encoder_levels.add_module(
                ...
                FFAEnhancedAdpWaveletBlock(configs, input_size)  # 使用FFA增强版本
            )
            in_planes *= 1
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, clusters):
        ...
        # Encoding with FFA enhanced wavelet blocks
