...
class ClusteredLinear(nn.Module):
    ...
    def forward(self, x, clusters):
        ...
        for ch in range(self.enc_in):
            cluster_id = str(int(clusters[ch]))
            ch_data = x[:, ch, :].unsqueeze(1)
            outputs.append(self.linear_layers[cluster_id](ch_data))
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.clusters = None
        self.trend_linear = ClusteredLinear(self.n_clusters, self.enc_in, self.seq_len, configs.pred_len)
    ...
    def _get_clusters(self, x_enc):
        ...
        x_cluster = x_enc.detach().permute(2, 0, 1).contiguous().view(self.enc_in, -1)
        if self.training or self.clusters is None:
            ...
            if torch.is_tensor(clusters):
                ...
                self.clusters = torch.tensor(clusters, device=x_enc.device, dtype=torch.long)
        return self.clusters
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, clusters):
        ...
        # 使用DUET线性提取器处理趋势分量
        moving_mean_out = self.trend_linear(moving_mean.permute(0,2,1), clusters).permute(0,2,1)
