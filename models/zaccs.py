...
class HWD1D(nn.Module):
    """
    1D Haar小波下采样模块 - 适配时序数据
    基于HWD模块，针对1D时序数据进行优化
    """
    def __init__(self, in_ch, out_ch):
        super(HWD1D, self).__init__()
        
        # 1D Haar小波变换的简化实现
        # 将2D的4个分量(LL, LH, HL, HH)适配为1D的2个分量(L, H)
    ...
    def haar_wavelet_1d(self, x):
        ...
        # Haar小波变换
        # 低频分量 (近似系数)
        low_freq = (x[:, :, ::2] + x[:, :, 1::2]) / 2.0  # (B, C, L//2)
        
        # 高频分量 (细节系数)
        high_freq = (x[:, :, ::2] - x[:, :, 1::2]) / 2.0  # (B, C, L//2)
    ...
    def forward(self, x):
        ...
        # 特征表示学习
        x_out = self.conv_bn_gelu(x_combined)  # (B, out_ch, L//2)
        
        return x_out

class HWDEncoder(nn.Module):
    """
    基于HWD的编码器 - 多级小波下采样
    """
    def __init__(self, configs):
        super(HWDEncoder, self).__init__()
        
        # 多级HWD下采样层
        self.hwd_layers = nn.ModuleList()
        current_dim = configs.d_model
        
        # 3级HWD下采样
        for i in range(3):
            self.hwd_layers.append(HWD1D(current_dim, current_dim))
        
        # 特征增强层
        self.feature_enhance = nn.Sequential(
            nn.Conv1d(current_dim, current_dim, kernel_size=3, padding=1, groups=current_dim),
            nn.BatchNorm1d(current_dim),
            nn.GELU(),
            nn.Conv1d(current_dim, current_dim, kernel_size=1),
            nn.BatchNorm1d(current_dim),
            nn.GELU(),
        )
    ...
    def forward(self, x, attn_mask=None):
        ...
        current_x = x_hwd
        for hwd_layer in self.hwd_layers:
            current_x = hwd_layer(current_x)
            hwd_features.append(current_x)
            if current_x.size(-1) < 4:  # 防止过度下采样
                break
        
        # 取最后一层的HWD特征
        if hwd_features:
            hwd_out = hwd_features[-1]  # (B, C, L')
        ...
        # 门控融合
        combined = torch.cat([hwd_out, transformer_out], dim=-1)
        gate = self.fusion_gate(combined)
        
        # 自适应融合
        output = gate * hwd_out + (1 - gate) * transformer_out
...
class Model(nn.Module):
    """
    ZACCS: AdaWaveNet + HWD1D (Haar Wavelet Downsampling)
    融合Haar小波下采样的时序预测模型
    """
    ...
    def __init__(self, configs):
        ...
        # HWD编码器
        self.encoder = HWDEncoder(configs)
