...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        # x: (B, C, L)
        max_result = self.maxpool(x).squeeze(-1)  # (B, C)
        avg_result = self.avgpool(x).squeeze(-1)  # (B, C)
        max_out = self.se(max_result)  # (B, C)
        avg_out = self.se(avg_result)  # (B, C)
        output = max_out + avg_out  # (B, C)
        return output.unsqueeze(-1)  # (B, C, 1)

class SpatialAttention(nn.Module):
    """
    CBAM空间注意力机制 - 适配1D时序数据
    """
    ...
    def forward(self, x):
        # x: (B, C, L)
        max_result, _ = torch.max(x, dim=1, keepdim=True)  # (B, 1, L)
        avg_result = torch.mean(x, dim=1, keepdim=True)  # (B, 1, L)
...
class CBAMBlock1D(nn.Module):
    ...
    def __init__(self, channel=512, reduction=16, kernel_size=7):
        ...
        # 融合MLP
        self.joint_channel = channel + 1  # channel + spatial(1)
        self.MLP = nn.Sequential(
            nn.Linear(self.joint_channel, channel // reduction, bias=False),
            nn.ReLU(),
            nn.Linear(channel // reduction, self.joint_channel, bias=False)
        )
        self.sigmoid = nn.Sigmoid()
        
        # 可学习的残差权重
        self.residual_weight = nn.Parameter(torch.tensor(0.8))
        self.channel_weight = nn.Parameter(torch.tensor(0.1))
    ...
    def forward(self, x):
        ...
        # 通道注意力
        Channel_x = self.ChannelAttention(x)  # (B, C, 1)
        
        # 空间注意力
        Spatial_x = self.SpatialAttention(x)  # (B, 1, L)
        Spatial_x_pooled = torch.mean(Spatial_x, dim=2, keepdim=True)  # (B, 1, 1)
        
        # 拼接通道和空间特征
        Channel_x_flat = Channel_x.squeeze(-1)  # (B, C)
        Spatial_x_flat = Spatial_x_pooled.squeeze(-1)  # (B, 1)
        CS_x = torch.cat([Channel_x_flat, Spatial_x_flat], dim=1)  # (B, C+1)
        
        # 通过MLP建立相关性
        CS_xx = self.MLP(CS_x)  # (B, C+1)
        
        # 拆分并生成权重
        Channel_weight = self.sigmoid(CS_xx[:, :C].unsqueeze(-1))  # (B, C, 1)
        Spatial_weight = self.sigmoid(CS_xx[:, C:].unsqueeze(-1))  # (B, 1, 1)
        
        # 应用注意力权重
        channel_out = x * Channel_weight  # (B, C, L)
        spatial_out = x * Spatial_weight.expand(-1, C, L)  # (B, C, L)
        
        # 加权融合
        output = (self.residual_weight * residual + 
                 self.channel_weight * channel_out + 
...
class MultiDWConv1D(nn.Module):
    ...
    def forward(self, x, seq_len):
        ...
        x1 = self.act1(self.bn1(x1)).transpose(1, 2)
        ...
        x2 = self.act2(self.bn2(x2)).transpose(1, 2)
        ...
        x3 = self.act3(self.bn3(x3)).transpose(1, 2)
        ...
        x = weights[0] * x1 + weights[1] * x2_interp + weights[2] * x3_interp
...
class CBAMBlock1D_MRFP(nn.Module):
    """
    融合CBAM通道空间注意力和MRFP多感受野处理的增强块
    """
    def __init__(self, channel=512, reduction=16, kernel_size=7, dropout=0.1, mrfp_ratio=2.0):
        super().__init__()
        ...
        # CBAM通道空间注意力
        self.cbam_attention = CBAMBlock1D(channel=channel, reduction=reduction, kernel_size=kernel_size)
    ...
    def forward(self, x):
        ...
        # CBAM通道空间注意力
        attention_out = self.cbam_attention(x_norm)
...
class Model(nn.Module):
    """
    YangChanel模型: AdaWaveNet + CBAM通道空间融合注意力 + MRFP多感受野处理
    不改变小波内部的基本结构，保持AdaWaveNet的核心架构
    """
    ...
    def __init__(self, configs):
        ...
        # 融合CBAM和MRFP的增强注意力模块
        self.cbam_mrfp_block = CBAMBlock1D_MRFP(
            channel=configs.enc_in, 
            reduction=getattr(configs, 'cbam_reduction', 16),
            kernel_size=getattr(configs, 'cbam_kernel_size', 7),
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, clusters):
        ...
        # 应用融合CBAM和MRFP的增强注意力机制
        x = self.cbam_mrfp_block(x)
