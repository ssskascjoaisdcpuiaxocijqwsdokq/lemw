...
result_dir="experiment_results/ETTm2_${model_name}_${timestamp}"
mkdir -p $result_dir
...
echo "========================================" | tee -a $log_file
echo "AdaWaveNet ETTm2 实验开始" | tee -a $log_file
...
# 实验1: ETTm2 96->96
echo "" | tee -a $log_file
echo "开始实验1: ETTm2 96->96 预测..." | tee -a $log_file
echo "开始时间: $(date)" | tee -a $log_file
...
if [ $? -eq 0 ]; then
    ...
    # 复制结果文件
    cp -r ./results/long_term_forecast_ETTm2_96_96_* "${result_dir}/exp1_results/" 2>/dev/null || echo "⚠️ 实验1结果文件复制失败" | tee -a $log_file
    cp -r ./checkpoints/long_term_forecast_ETTm2_96_96_* "${result_dir}/exp1_checkpoints/" 2>/dev/null || echo "⚠️ 实验1模型文件复制失败" | tee -a $log_file
...
# 实验2: ETTm2 96->192
echo "" | tee -a $log_file
echo "开始实验2: ETTm2 96->192 预测..." | tee -a $log_file
echo "开始时间: $(date)" | tee -a $log_file
...
if [ $? -eq 0 ]; then
    ...
    # 复制结果文件
    cp -r ./results/long_term_forecast_ETTm2_192_192_* "${result_dir}/exp2_results/" 2>/dev/null || echo "⚠️ 实验2结果文件复制失败" | tee -a $log_file
    cp -r ./checkpoints/long_term_forecast_ETTm2_192_192_* "${result_dir}/exp2_checkpoints/" 2>/dev/null || echo "⚠️ 实验2模型文件复制失败" | tee -a $log_file
...
# 实验3: ETTm2 96->336
echo "" | tee -a $log_file
echo "开始实验3: ETTm2 96->336 预测..." | tee -a $log_file
echo "开始时间: $(date)" | tee -a $log_file
...
if [ $? -eq 0 ]; then
    ...
    # 复制结果文件
    cp -r ./results/long_term_forecast_ETTm2_336_336_* "${result_dir}/exp3_results/" 2>/dev/null || echo "⚠️ 实验3结果文件复制失败" | tee -a $log_file
    cp -r ./checkpoints/long_term_forecast_ETTm2_336_336_* "${result_dir}/exp3_checkpoints/" 2>/dev/null || echo "⚠️ 实验3模型文件复制失败" | tee -a $log_file
...
# 实验4: ETTm2 96->720
echo "" | tee -a $log_file
echo "开始实验4: ETTm2 96->720 预测..." | tee -a $log_file
echo "开始时间: $(date)" | tee -a $log_file
...
if [ $? -eq 0 ]; then
    ...
    # 复制结果文件
    cp -r ./results/long_term_forecast_ETTm2_96_720_* "${result_dir}/exp4_results/" 2>/dev/null || echo "⚠️ 实验4结果文件复制失败" | tee -a $log_file
    cp -r ./checkpoints/long_term_forecast_ETTm2_96_720_* "${result_dir}/exp4_checkpoints/" 2>/dev/null || echo "⚠️ 实验4模型文件复制失败" | tee -a $log_file
...
echo "AdaWaveNet ETTm2 实验总结" > $summary_file
echo "=========================" >> $summary_file
...
# 复制总结到主目录
cp $summary_file "./ETTm2_experiment_summary_${timestamp}.txt"

echo ""
echo "🎉 所有实验已完成并保存!"
echo "📁 详细结果保存在: $result_dir"
echo "📊 实验总结: ./ETTm2_experiment_summary_${timestamp}.txt"
echo ""
